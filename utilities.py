...
def train_one_epoch(model, dataloader, loss_fn, optimizer, epoch, fold, device):
    ...
    for batch_num, batch in enumerate(tqdm(dataloader)):
        ...
        y_pred_batch = torch.argmax(output, dim=1).numpy()
        ...
        if batch_num % 10 == 9:
            ...
            running_loss = 0.
            wandb.log(data={'Epoch': epoch, f'Training_loss_{fold}': avg_loss})

        # Stack
        y_train = np.hstack([y_train, y_batch.numpy()]) if y_train.size else y_batch.numpy()
